...
@APICommand(name = "addBulkUsersToProject", description = "Adds multiple users to a project in a single bulk operation", responseObject = SuccessResponse.class, since = "4.20",
        requestHasSensitiveInfo = false, responseHasSensitiveInfo = false,
        authorized = {RoleType.Admin, RoleType.DomainAdmin, RoleType.ResourceAdmin, RoleType.User})
public class AddBulkUsersToProjectCmd extends BaseAsyncCmd {
    ...
    @Parameter(name = ApiConstants.PROJECT_ID,
            ...
            required = true,
            description = "ID of the project to add the users to")
    private Long projectId;

    @Parameter(name = "usernames",
            type = CommandType.LIST,
            collectionType = CommandType.STRING,
            required = true,
            description = "Comma-separated list of usernames to be added to the project")
    private List<String> usernames;

    @Parameter(name = ApiConstants.PROJECT_ROLE_ID, type = BaseCmd.CommandType.UUID, entityType = ProjectRoleResponse.class,
            description = "ID of the project role to assign to all users", validations = {ApiArgValidator.PositiveNumber})
    private Long projectRoleId;

    @Parameter(name = ApiConstants.ROLE_TYPE, type = BaseCmd.CommandType.STRING,
            description = "Project role type to be assigned to all users - Admin/Regular")
    ...
    public String getEventDescription() {
        return "Adding users " + String.join(", ", usernames) + " to project: " + getResourceUuid(ApiConstants.PROJECT_ID);
    }
    ...
    public void execute() {
        validateInput();
        boolean result = bulkProjectUsersService.addBulkUsersToProject(getProjectId(), getUsernames(), getProjectRoleId(), getRoleType());
        if (result) {
            SuccessResponse response = new SuccessResponse(getCommandName());
            this.setResponseObject(response);
        } else {
            throw new ServerApiException(ApiErrorCode.INTERNAL_ERROR, "Failed to add users to the project");
        }
    }

    private void validateInput() {
        if (usernames == null || usernames.isEmpty()) {
            throw new InvalidParameterValueException("Must specify at least one username");
        }
        if (getProjectId() < 1L) {
